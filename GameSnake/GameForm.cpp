...
void GameSnake::GameForm::GeneratePositionFruit()
{
	...
	positionFruit.X = rand->Next(10, gameArea.X-10);//10 - смещение относительно боков формы
	positionFruit.Y = rand->Next(90, gameArea.Y-10);//90 - координаты верхней границы относительно верха формы
...
{
	//Проверим позицию головы и позицию фрукта
...
{
	//Если инициация была - удаляем объекты с формы
	if (!firstLaunch) {
	...
	Serpens[0]->BackColor = Color::Green;//Голова будет зеленой
...
void GameSnake::GameForm::ChackBorders()
{
	...
	labelCoordinatesSnake->Text = "Координаты змейки: (" + Convert::ToString(Serpens[0]->Location.X) + ", " + Convert::ToString(Serpens[0]->Location.Y) + ")";
}
//реализация событий
...
System::Void GameSnake::GameForm::паузаToolStripMenuItem_Click(System::Object^ sender, System::EventArgs^ e)
{
	if (play) {
		play = false;
	}
...
System::Void GameSnake::GameForm::информацияОбИгреToolStripMenuItem_Click(System::Object^ sender, System::EventArgs^ e)
{
	MessageBox::Show("Правила игры:\n1. Для управления использовать стрелки\n2. Ешьте фрукты для набора счета и увеличения змейки\n3. Нельзя есть себя и врезаться в стенку", "Правила игры!");
...
void GameSnake::GameForm::GameForm_Update(Object^ obgect, EventArgs^ e)
{
	if (!die && play) {
		...
		Eating();//Проверяем что съели фрукт
		EatYourself();//Проверка что съел себя
		ChackBorders();//Проверка на столкновение со стеной
	}
	else if (die && play) {
		timer->Stop();
